...
final class CodableFeedStoreTests: XCTestCase {
    ...
    func test_retrieveAfterInsertingToEmptyCache_deliversNonEmptyOnNonEmptyCache() {
        let sut = makeSUT()
        let exp = expectation (description: "Wait for cache retrieval")
        let feed = uniqueImageFeed().local
        let timestamp = Date()
        sut.insert(feed, timestamp: timestamp) {
            insertionError in
            XCTAssertNil(insertionError,
                         "Expected feed to be inserted successfully")
        ...
        sut.insert(feed, timestamp: timestamp) {
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail("Expected found result with feed \(feed) and timestamp \(timestamp),got \(firstResult) and \(secondResult) instead")
                    }
                }
                exp.fulfill()
            }
